...
2. Under **Identifying and authorizing users**, set
   - **User authorization callback URL**: `${broker-url}/copilot-oauth/callback`
     (e.g. `https://api.lava.magmamoose.com/copilot-oauth/callback`)
...
```bash
curl https://api.lava.magmamoose.com/copilot-oauth/status?user=CalebSargeant
...
curl -fsSL -X POST \
  ...
  https://api.lava.magmamoose.com/copilot-quota
```
